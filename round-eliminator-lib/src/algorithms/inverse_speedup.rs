...
use crate::{problem::Problem, constraint::Constraint, group::{Label, Group}};
    ...
    pub fn inverse_speedup(&self) -> Self {
        ...
        for line in self.active.all_choices(false) {
            let mut newline = line.edited(|g|{ 
                ...
                next_label += 1;
                mapping_oldlabel_labels.entry(label).or_default().push(new_label);
                Group(vec![new_label])
            });
            newline.normalize();
        ...
        let active = self.passive.edited(|g|{
            Group(g.iter().flat_map(|label|{
                mapping_oldlabel_labels.get(label).unwrap_or(&empty).iter().cloned()
